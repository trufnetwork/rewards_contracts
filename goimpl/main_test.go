...
func TestMerkleTree(t *testing.T) {
	...
	t.Run("genMerkleTree with 3 leafs", func(t *testing.T) {
		mt, err := genRewardMerkleTree([]string{addr1, addr2, addr3}, []string{"100", "200", "100"}, contract)
		require.NoError(t, err)
		mtFile, err := os.ReadFile("../test/testdata/3leafs_tree.json")
		require.NoError(t, err)
	...
	t.Run("genMerkleTree with 4 leafs", func(t *testing.T) {
		mt, err := genRewardMerkleTree([]string{addr1, addr2, addr3, addr4}, []string{"100", "200", "100", "200"}, contract)
		require.NoError(t, err)
		mtFile, err := os.ReadFile("../test/testdata/4leafs_tree.json")
		require.NoError(t, err)
		assert.JSONEq(t, mt, string(mtFile))
		mtProof, mtLeaf, err := getMTreeProof(mt, addr3)
		require.NoError(t, err)
	...
	t.Run("genMerkleTree with 5 leafs", func(t *testing.T) {
		mt, err := genRewardMerkleTree([]string{addr1, addr2, addr3, addr4, addr5}, []string{"100", "200", "100", "200", "100"}, contract)
		require.NoError(t, err)
		mtFile, err := os.ReadFile("../test/testdata/5leafs_tree.json")
		require.NoError(t, err)
		assert.JSONEq(t, mt, string(mtFile))
		mtProof, mtLeaf, err := getMTreeProof(mt, addr3)
		require.NoError(t, err)
